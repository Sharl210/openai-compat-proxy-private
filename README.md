...
- `RAW_MODEL_NAME_REPLACE`：模型名的**字符串替换**，provider 级，承担双向职责——**响应/展示方向**把上游原始名替换成伪原始名（`/models` 展示、响应模型名），**请求方向**把客户端发的伪原始名还原成原始名发上游。它只是字符串层面的替换，不解析任何模型语义（不涉 reasoning suffix、`-pro/-auto/-ultra`、`-noprompt`、推理家族、provider 标记等），也不改变模型映射、准入或限制规则的匹配基准。规则逗号分隔、按顺序逐条应用：字面量替换 `old:new`（把模型名中所有出现的 `old` 替换为 `new`，例如 `quectel-github-copilot/:relay/`），正则替换 `#re:pattern:replacement`（Go regexp 对模型名做**子串匹配**，把命中的每个子串替换为 `replacement`；`replacement` 支持 `$0-$9` 捕获占位、`\$` 字面美元，`$10` 及以上保持字面值）。例如 `RAW_MODEL_NAME_REPLACE=#re:.*quectel-github-copilot/(.*):$1` 会把原始名 `quectel-github-copilot/QDeepseekV4/deepseek-v4-flash` 替换成伪原始名 `QDeepseekV4/deepseek-v4-flash` 展示；客户端用伪原始名请求时，代理会把它还原成原始名发上游。请求方向的还原绑定模型列表索引生命周期（配置热加载 / 索引刷新时同步更新映射）。值中禁止使用 `<<` 或 `>>`（专用于 `MODEL_MAP` / `V1_MODEL_MAP` 的 provider 标记）。留空表示不启用替换；该字段可热加载，修改后无需重启。
