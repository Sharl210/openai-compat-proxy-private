...
func TestManagerFlushAll_WritesDefaultGroupAggregateTXT(t *testing.T) {
	tmp := t.TempDir()
	loc := time.FixedZone("CST", 8*3600)
	clock := newMockClock(loc)
	...
	m.SetDefaultProvidersSource(func() []string {
		return []string{"anthropic", "missing", "disabled", "openai"}
	})

	if err := m.RecordFinalUsage("req-openai", "openai", &Usage{InputTokens: 100, TotalTokens: 100}); err != nil {
		t.Fatalf("RecordFinalUsage openai: %v", err)
	}
	if err := m.RecordFinalUsage("req-anthropic", "anthropic", &Usage{InputTokens: 40, TotalTokens: 40}); err != nil {
		t.Fatalf("RecordFinalUsage anthropic: %v", err)
	}
	if err := SaveProviderStats(tmp, "disabled", &ProviderStats{
		Timezone:     loc.String(),
		RecentDays:   []DailyStats{{Date: "2026-03-27", Totals: TokenTotals{InputTokens: 999, TotalTokens: 999, RequestCount: 9}}},
		HistoryTotal: TokenTotals{InputTokens: 999, TotalTokens: 999, RequestCount: 9},
		UpdatedAt:    time.Date(2026, 3, 27, 8, 0, 0, 0, loc),
	}); err != nil {
		t.Fatalf("SaveProviderStats disabled: %v", err)
	}

	m.flushAll()

	defaultTXT, err := os.ReadFile(expectedAggregateCacheInfoTXTPath(tmp, "v1默认分组统计.txt"))
	if err != nil {
		t.Fatalf("read default group aggregate txt: %v", err)
	}
	defaultStr := string(defaultTXT)
	for _, want := range []string{"[今天]", "输入Tokens：140", "总计Tokens：140", "总调用次数：2", "[提供商历史以来总计]"} {
		if !strings.Contains(defaultStr, want) {
			t.Fatalf("default group aggregate txt missing %q:\n%s", want, defaultStr)
		}
	}
	if strings.Contains(defaultStr, "输入Tokens：1139") {
		t.Fatalf("default group aggregate should exclude disabled provider totals:\n%s", defaultStr)
	}
}
