...
type Config struct {
	...
	DefaultProReasoningModeSet            bool
...
func loadFromLookup(lookup func(string) (string, bool)) Config {
	...
	if value, ok := lookup("ENABLE_REASONING_MODE_SUFFIX"); ok && value != "" {
		cfg.EnableReasoningModeSuffix = parseRootBool(value)
		cfg.EnableReasoningModeSuffixSet = true
	}
	if value, ok := lookup("DEFAULT_PRO_REASONING_MODE"); ok && value != "" {
		cfg.DefaultProReasoningMode = parseRootBool(value)
		cfg.DefaultProReasoningModeSet = true
	}
	if value, ok := lookup("DEFAULT_PRO_REASONING_MODE_EXCLUDED_MODELS"); ok && value != "" {
		if rules, err := parseModelPatternRules(value, "DEFAULT_PRO_REASONING_MODE_EXCLUDED_MODELS", "root env DEFAULT_PRO_REASONING_MODE_EXCLUDED_MODELS"); err == nil {
			cfg.DefaultProReasoningModeExcludedModels = rules
		}
	}
	if value, ok := lookup("ULTRA_MAX_CONCURRENT_SUBAGENTS"); ok && value != "" {
...
func (c Config) hotReloadableRootEquals(other Config) bool {
	return c.ProxyAPIKey == other.ProxyAPIKey &&
		...
		c.EffectiveEnableReasoningModeSuffix() == other.EffectiveEnableReasoningModeSuffix() &&
