...
func TestResponsesRouteAllowsEncryptedMetadataOnRepresentableToolItemsForNonResponsesUpstream(t *testing.T) {
	for _, endpointType := range []string{config.UpstreamEndpointTypeChat, config.UpstreamEndpointTypeAnthropic} {
		t.Run(endpointType, func(t *testing.T) {
			upstreamHits := 0
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				w.Header().Set("Content-Type", "application/json")
				if endpointType == config.UpstreamEndpointTypeAnthropic {
					_, _ = w.Write([]byte(`{"id":"msg_tool_metadata","type":"message","role":"assistant","content":[{"type":"text","text":"ok"}],"stop_reason":"end_turn","usage":{"input_tokens":1,"output_tokens":1}}`))
					return
				}
				_, _ = w.Write([]byte(`{"id":"chat_tool_metadata","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2}}`))
			}))
			defer upstream.Close()
			...
			req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{
				"model":"gpt-5",
				"input":[
					{"type":"function_call","call_id":"call_1","name":"lookup","arguments":"{}","encrypted_content":"transport-metadata"},
					{"type":"function_call_output","call_id":"call_1","output":"{\"ok\":true}","encrypted_content":"transport-metadata"}
				]
			}`))
			req.Header.Set("Content-Type", "application/json")
			rec := httptest.NewRecorder()
			...
			if rec.Code != http.StatusOK || upstreamHits != 1 {
				t.Fatalf("expected representable tool items to reach %s upstream, status=%d hits=%d body=%s", endpointType, rec.Code, upstreamHits, rec.Body.String())
			}
		})
	}
}
