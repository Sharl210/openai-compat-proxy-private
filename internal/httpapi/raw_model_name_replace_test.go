...
func TestRawModelNameReplaceAppliedBeforeUpstreamAcrossProtocols(t *testing.T) {
	...
	for _, entrypoint := range tests {
		t.Run(entrypoint.name, func(t *testing.T) {
			...
			// MODEL_MAP target "middle-model" -> literal middle->replaced gives
			// "replaced-model" -> regexp appends -v2 to the trailing "model".
			if got := rec.Header().Get(headerProxyToUpstreamModel); got != "replaced-model-v2" {
				t.Fatalf("expected replaced transparency model, got %q", got)
			}
			if got := upstreamPayload["model"]; got != "replaced-model-v2" {
				t.Fatalf("expected upstream model replaced, got %#v", upstreamPayload)
			}
		})
	}
}
...
func TestRawModelNameReplaceKeepsLimitRulesOnPreReplacementName(t *testing.T) {
	...
	if got := rec.Header().Get(headerProxyToUpstreamModel); got != "replaced-model" {
		t.Fatalf("expected replaced transparency model, got %q", got)
	}
}
