...
func TestManagedTransportPreservesRoundTripErrorAndNilResponse(t *testing.T) {
...
func TestManagedTransportPreservesNilBody(t *testing.T) {
...
func TestManagedTransportWrapsNonNilBodyAndPreservesResponse(t *testing.T) {
...
func TestManagedTransportRetireIsImmediateAndIdempotent(t *testing.T) {
...
func TestManagedTransportRejectsRoundTripAfterRetirement(t *testing.T) {
	var roundTrips atomic.Int64
	transport := newManagedTestTransport(func(*http.Request) (*http.Response, error) {
		roundTrips.Add(1)
		return &http.Response{Body: io.NopCloser(strings.NewReader("unexpected"))}, nil
	})
	...
	if response != nil {
		...
		t.Fatalf("retired RoundTrip returned response: %v", response)
	}
	if err == nil {
		t.Fatal("retired RoundTrip unexpectedly succeeded")
	}
	if got := roundTrips.Load(); got != 0 {
		t.Fatalf("underlying RoundTrip calls=%d, want 0", got)
	}
}

func TestManagedTransportRetiredBodyTerminalClosesIdleOnce(t *testing.T) {
	tests := []struct {
		name   string
		reads  []managedTestRead
...
func TestManagedTransportTerminalBeforeRetirementOnlyClosesOnRetirement(t *testing.T) {
...
func TestManagedTransportConcurrentRoundTripAndRetireClosesAfterTerminal(t *testing.T) {
...
func TestManagedTransportPreservesBodyReadAndCloseErrors(t *testing.T) {
